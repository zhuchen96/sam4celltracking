...
from utils.tools import track_continuity_of_elements, find_closest_nonzero_and_keep_region_3d, get_prompt_points_3d, link_existing_cells, extract_tiff_3d, get_centriod
...
def main(args): 
    ...
    for i in range(start_tf, end_tf):
        ...
        # Update image in SAM3D
        segmenter.update_image(reversed_index)
        # First time frame
        if i == 0:
            ...
            for current_ind in temp_dict.keys():
                ...
                value = mask_arr[z, y, x]
                if value == 0: # If there is no directly linked mask, use sam3D to generate a new mask
                    # Click using SAM3D, result in an array of size 128 x 128 x 128, containing the centered segmentation mask
                    new_array = segmenter.segmentation([z, y, x])

                    # Find masks indices, index of local position
                    indices = np.argwhere(new_array == 1)
                    if indices.shape[0] == 0:
                        del_id.append(current_ind)
                        print(f"Tracking of cell {current_ind} is stopped. No 3D mask")
                        continue

                    # Find masks indices, index of global position
                    ...
                    y = center_of_mass[1]
                    z = center_of_mass[0]
                    # Don't check for CHO, because it only contains few slices in z axis
                    if "Fluo-N3DH-CHO" not in image_path:
                    ...
                    # Terminate tracking when the cell is away or too small
                    if save_indices.shape[0] <= dis_threshold:
                        del_id.append(current_ind)
                        print(f"Tracking of cell {current_ind} is stopped. Mask is too small!")
                        continue
                    # Try to link cell based on overlapping region
                    save_indices, center_of_mass, value = link_existing_cells(mask_arr, save_indices, image_z, image_y, image_x)
                else:
