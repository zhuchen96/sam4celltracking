...
from utils.tools import extract_tiff, get_prompt_points, get_centriod, get_prompt_box, find_closest_nonzero_and_keep_region_3d, find_closest_nonzero_value_and_distance, track_continuity_of_elements

# Prediction shift in two adjacent frames
def get_shift(predictor, pos, pts_list, bb_points, img_list, reversed_ind, mask_ind, tmp_mask, size, image_y, image_x, neg_num, img_path):
    ...
    patches = [extract_tiff(p, start_y, end_y, start_x, end_x, overlay_size, image_y, image_x) for p in reversed(img_list)]
    ...
    true_indices = np.argwhere(video_segments[1][1])
    '''
    if visualize:
        plt.figure(figsize=(12, 8))
        plt.title(f"frame {ann_frame_idx}")
    ...
    return np.mean(true_indices, axis=0)[1:], tmp_mask, out_mask
...
def main(args): 
    ...
    image_y = init_mask.shape[0]
    image_x = init_mask.shape[1]
    ...
    for i in range(start_tf, end_tf):
        ...
        mask = np.zeros((image_y, image_x), dtype=np.uint16)
        ...
        if i == 0:
            ...
            for current_ind, position in enumerate(positions):
                ...
                # Get positive / negative prompt points and bounding box based on the mask
                bb_points = get_prompt_box(save_indices, [y, x])
                slice_pts = get_prompt_points(neg_num, size, save_indices, [y, x])
            ...
            for current_ind in temp_dict.keys():
                if current_ind % 50 == 0:
                    print(f"Tracking cell: {current_ind:03d}")
                # Get centroid
                y = temp_dict[current_ind]['centroid'][0]
                x = temp_dict[current_ind]['centroid'][1]
                ...
                if slice_pts is not None:
                    shift_vec, tmp_mask, out_mask = get_shift(sam2, [y, x], slice_pts, bb_points, images[reversed_index:reversed_index+2], reversed_index, current_ind, tmp_mask, size, image_y, image_x, neg_num, image_path)
                    ...
                    if np.isnan(shift_vec).any():
                        ...
                        shift_vec = [size/2, size/2]       

                    # Shift the centroid, update the click position
                    x = x + int(shift_vec[1] - size/2)
                    y = y + int(shift_vec[0] - size/2)
                
                # Find the closest mask based on the shifted centroid
                closest_value, distance = find_closest_nonzero_value_and_distance(mask_arr, (y, x))
                if distance <= neighbor_dist:
                    value = closest_value # Mask is linked 
                else:
                    value = 0 # Mask is not linked

                if value != 0: # If linked
                    save_indices = np.argwhere(mask_arr==value)
                    [y, x] = np.mean(save_indices, axis=0)
                    y = int(y)
                    x = int(x)

                    if len(existing_values_dict[value]) == 0: # If linked mask is unique
                        existing_values_dict[value].append(current_ind)
                    elif len(existing_values_dict[value]) == 1: # If the mask is already linked to another daughter -> mitosis
                        existing_values_dict[value].append(current_ind)
                        del_id.append(existing_values_dict[value][0])
                        del_id.append(current_ind)
                        mask[mask==existing_values_dict[value][0]] = 0
                        # Assign a new label to the parent cell
                        new_ind += 1
                        # Save daughter cells to mitosis dict
                        ...
                        # Get prompt for the parent cell
                        bb_points = get_prompt_box(save_indices, [y, x])
                        slice_pts = get_prompt_points(neg_num, size, save_indices, [y, x])
                        if slice_pts is not None:
                            # Transform the coordinates to global coordinates
                            slice_pts = slice_pts + [y, x]
                            bb_points = np.array(bb_points) + np.array([y, x, y, x])
                        # Save parent cell in a new dictionary
                        new_dict[new_ind] = {}
                        new_dict[new_ind]['centroid'] = [y, x]
                        new_dict[new_ind]['centroid_old'] = [y_old, x_old]                
                        new_dict[new_ind]['slice_pts'] = slice_pts
                        new_dict[new_ind]['tmp_mask'] = tmp_mask
                        new_dict[new_ind]['bb_points'] = bb_points       
                    else:
                        # Three cells linked to the same parent, stop tracking the cell (need improvement)
                        del_id.append(current_ind)
                else: # If mask is not linked
                    out_ind = np.argwhere(out_mask==True) # Use SAM2 output mask
                    if out_ind is None: # No output mask from SAM2
                        del_id.append(current_ind)
                        print(f"Tracking of cell {current_ind} is stopped. No linking mask and not trackable")   
                        continue     
                    else: # Keep SAM2 output mask               
                        save_indices = out_ind[:, 1:]
                        save_indices = save_indices + [y, x] - [int(size/2), int(size/2)]
                        save_indices = save_indices[(save_indices[:, 0] >= 0) & (save_indices[:, 1] >= 0 ) & (save_indices[:, 0] < image_y -1) & (save_indices[:, 1] < image_x - 1)]
                        print(f"Tracking of cell {current_ind} is not linked!")  
                    # Stop tracking is mask is too small      
                    if save_indices.shape[0] <= dis_threshold:
                        del_id.append(current_ind)
                        print(f"Tracking of cell {current_ind} is stopped. Mask is too small!")
                        continue

                    # Stop tracking when the cell is away
                    if x <=5 or y <= 5:
                        del_id.append(current_ind)
                        print(f"Tracking of cell {current_ind} is stopped. Cell moves away!")
                        continue
                    if y >= image_y - 5 or x >= image_x - 5:
                        del_id.append(current_ind)
                        print(f"Tracking of cell {current_ind} is stopped. Cell moves away!")
                        continue
                ...
                if current_ind not in new_dict:
                    ...
                    temp_dict[current_ind]['centroid'] = [y, x]
                    temp_dict[current_ind]['centroid_old'] = [y_old, x_old]                
                    temp_dict[current_ind]['slice_pts'] = slice_pts
                    temp_dict[current_ind]['tmp_mask'] = tmp_mask
                    temp_dict[current_ind]['bb_points'] = bb_points       
            # Delete cells
            ...
            for key, value in existing_values_dict.items():
                if value == []: # Unlinked mask
                    ...
                    mask[tuple(save_indices.T)] = current_ind
                    # Get prompt points (local coordinates)
                    slice_pts = get_prompt_points(neg_num, size, save_indices, center_of_mass)
                    bb_points = get_prompt_box(save_indices, [y, x])
                    if slice_pts is not None:
                        # Transform the coordinates to global coordinates
                        slice_pts = slice_pts + [y, x]
        ...
        for value in temp_dict.keys():
            if int(value) not in mask:
                add_x = temp_dict[value]['centroid'][1]
                add_y = temp_dict[value]['centroid'][0]
                print(f"Add dot of cell {int(value)}")
                while [add_y, add_x] in added_pos:
                    add_y -= 1
                    add_x -= 1
                mask[add_y, add_x] = value
                added_pos.append([add_y, add_x])
    ...
    # Change to 04d if it's a long sequence
    if longer_sequence:
        ...
        for filename in os.listdir(folder_path):
            if filename.startswith("mask") and filename.endswith(".tif") and len(filename) == 11:
                # Extract the integer part of the filename
                num_str = filename[4:-4]
                
                # Convert to an integer and reformat it to 4 digits
                new_num_str = f"{int(num_str):04d}"
                
                # Create the new filename
                new_filename = f"mask{new_num_str}.tif"
